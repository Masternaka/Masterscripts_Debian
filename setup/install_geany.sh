#!/usr/bin/env bash
# DESC: Install Geany text editor - choice of APT package (2.0) or ButterRepo (2.1)
...
# Function to install Geany from ButterRepo
install_geany_butterrepo() {
    echo -e "${CYAN}Installing Geany 2.1 from ButterRepo...${NC}"
    echo -e "${YELLOW}This will install the latest version with all plugins.${NC}"
    echo

    # Add butterrepo if not already configured
    SCRIPT_DIR_GEANY="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    if [ -f "$SCRIPT_DIR_GEANY/add_butterrepo.sh" ]; then
        bash "$SCRIPT_DIR_GEANY/add_butterrepo.sh"
    else
        wget -qO /tmp/add_butterrepo.sh "https://justaguy.dev/drew/butterscripts/raw/branch/main/setup/add_butterrepo.sh"
        bash /tmp/add_butterrepo.sh
    fi

    echo -e "${YELLOW}Installing geany-latest...${NC}"
    sudo apt install -y geany-latest

    echo -e "${GREEN}Geany 2.1 installed successfully!${NC}"
    echo -e "${YELLOW}Includes 46 plugins (markdown, git-changebar, treebrowser, etc.)${NC}"
...
apply_butterscripts_config() {
    ...
    EXISTING_SCHEMES=$(find "$COLORSCHEMES_DIR" -maxdepth 1 -name '*.conf' 2>/dev/null | wc -l)
...
echo
echo -e "${CYAN}1.${NC} Install from ButterRepo (Geany 2.1) ${GREEN}(RECOMMENDED)${NC}"
echo -e "   - Newest features (Dart, Docker, Zig language support)"
echo -e "   - 46 plugins included"
echo -e "   - Fast install, no compilation needed"
echo
echo -e "${CYAN}2.${NC} Install from APT"
