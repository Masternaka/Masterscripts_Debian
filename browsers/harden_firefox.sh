#!/usr/bin/env bash
# DESC: Harden Firefox/Firefox-ESR with privacy-focused settings
...
# Where the merged user.js (Betterfox + Butterbian additions) is cached.
...
# Fetch pinned Betterfox + append Butterbian additions → cached user.js
generate_userjs() {
    ...
    {
        cat <<HEADER
// Butterbian Firefox hardening — Betterfox ${BETTERFOX_TAG} + Butterbian additions
// Generated by butterscripts/browsers/harden_firefox.sh
...
// ============================================================================
// BUTTERBIAN ADDITIONS — Linux-targeted, intentional, minimal
// ============================================================================
...
# Keeps user.js current on every launch (covers CLI, sxhkd, WM keybinds, rofi)
create_wrapper_script() {
    local exec_name="$1"
    local profile_name="$2"  # e.g., default-release
...
show_menu() {
    ...
    if ! $firefox_installed && ! $firefox_esr_installed; then
        ...
        echo -e "${YELLOW}Install Firefox first using install_browsers.sh${NC}"
        exit 1
    fi
    ...
    echo -e "  - Apply Betterfox ${BETTERFOX_TAG} + Butterbian additions (user.js)"
